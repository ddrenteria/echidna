...
run_amount = 10
# Contracts to run
contracts = ["BitMath", "FullMath", "LowGasSafeMath", "Oracle", "SqrtPriceMath", "SwapMath", "TickBitmap", "Tick",
...
def get_test_path_base(testLimit, timeout_minutes, seqLen, contractName):
    return f"echidna-test/uniswap/seqLen_{seqLen}_timeout_{timeout_minutes}_testLim_{testLimit}/{contractName}"
...
def run_echidnas(testLimit, timeout_minutes, seqLen, contract_name, iteration):
    ...
    contract_dir = "uniswap/test/" + contract_name + "EchidnaTest.sol"
    ...
    our_run_command = f"echidna {contract_dir} --contract {contract_name}EchidnaTest --config config.yaml > {path_ours}/script_output.txt"
    og_run_command = f"echidna {contract_dir} --contract {contract_name}EchidnaTest --config config_og.yaml > {path_og}/script_output.txt"
    random_run_command = f"echidna {contract_dir} --contract {contract_name}EchidnaTest --config config_random.yaml > {path_random}/script_output.txt"

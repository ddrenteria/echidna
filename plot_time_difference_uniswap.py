...
# Mazes to run
mazes = ["BitMath", "FullMath", "LowGasSafeMath", "Oracle", "SqrtPriceMath", "SwapMath", "TickBitmap", "Tick",
            "TickMath", "TickOverflowSafety", "UnsafeMath"]
# Test limits to use 
test_limits = [2000000]
...
def get_test_path_base(testLimit, timeout_minutes, seqLen, mazeNumber):
    return f"echidna-test/uniswap/seqLen_{seqLen}_timeout_{timeout_minutes}_testLim_{testLimit}/{mazeNumber}"
...
def create_plot_coverage_points_seq_number(df_ours, df_og, path, seqLen):
    df_ours = df_ours[df_ours["coverage_points"]>0]
    df_og = df_og[df_og["coverage_points"]>0]
    df_random = df_random[df_random["coverage_points"]>0]
...
def create_plot_average(df_ours, df_og, path, seqLen):
...
def create_data(df_ours, df_og, ar_our, ar_og):
    og = [float(i) for i in df_og["time"]][-1]
    our = [float(i) for i in df_ours["time"]][-1]
    ar_our.append((our - og)/og*100)
    return ar_our, ar_og
...
def plot_data_all(ar_our, ar_og, ar_random, labels):
    ...
    ax.set_ylabel('# tests')
    ax.set_xlabel('')
    ax.set_title('# Tests discovered by contract, grouped by Maze')
    ...
    plt.savefig("comparison_maze_test.jpg")
    plt.clf()
    #plt.show()
...
def plot_data_afl_og(ar_our, ar_og, labels):
    x = np.arange(len(labels))  # the label locations
    width = 0.3  # the width of the bars

    fig, ax = plt.subplots(figsize=(18,5))
    rects1 = ax.bar(x, ar_our, 2 * width, label='EchidnaAfl', color="cadetblue")
    # rects3 = ax.bar(x + (width/2), ar_og, width-0.02, label='Original', color="forestgreen")

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Overhead percentage')
    ax.set_xlabel('')
    ax.set_title('Overhead percentage of EchidnaAfl over original')
    ax.legend(title='Version')
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ...
    fig.tight_layout()
    # rcParams['figure.figsize'] = 15, 5
    #plt.savefig("comparison_afl_maze_test.jpg")
    #plt.clf()
...
for test_limit in test_limits:
    for seqLen in seqLens:
        for maze in mazes:
            ...
            labels.append(str(maze))
            create_data(df_ours, df_og, ar_ours, ar_og)
        labels.append("SeqLen " + str(seqLen))
        ar_og.append(0)
        ar_ours.append(0)
        
# plot_data_all(ar_ours, ar_og, ar_random, labels)
plot_data_afl_og(ar_ours, ar_og, labels)
